...
export default function RedefinirSenhaPage() {
  ...
  const formValido = regras.min && regras.numero && regras.maiuscula && regras.especial && regras.match && regras.minuscula;
    ...
    <div className="min-h-screen bg-white flex items-center justify-center p-6 relative overflow-hidden text-left">
      ...
      <div className="w-full max-w-md z-10">
        <div className="text-center mb-10">
          <Link href="/"><img src="/logo_azul.png" alt="Logo" className="h-50 mx-auto mb-6" /></Link>
          <h2 className="text-3xl font-black text-slate-900 tracking-tighter">Nova Senha</h2>
          <p className="text-slate-500 mt-2 font-medium italic text-sm">Crie uma senha mestre para sua segurança.</p>
        </div>

        <div className="bg-white/80 backdrop-blur-2xl p-8 rounded-[32px] border border-slate-100 shadow-xl">
          {!sucesso ? (
            <form onSubmit={handleReset} className="space-y-6 animate-in slide-in-from-right-4 duration-500">
              ...
              <div>
                <label className="block text-xs font-black text-slate-400 uppercase tracking-widest mb-2 ml-1">Confirmar Senha</label>
                <div className="relative group">
                  <input 
                    required 
                    ...
                    className={`w-full px-5 py-4 rounded-2xl bg-slate-50 border outline-none font-medium transition-all pr-12 group-hover:bg-white
                      ${novaSenha.confirmar && !regras.match ? 'border-rose-300' : 'border-slate-100 focus:border-cyan-500'}`}
                    placeholder="••••••••"
                  />
                  <button 
                    type="button"
                ...
                {novaSenha.confirmar && !regras.match && (
                  <p className="text-[10px] text-rose-500 font-black uppercase mt-2 ml-1 italic tracking-widest">As senhas não coincidem</p>
                )}
              </div>
              ...
              <button 
                ...
                className={`w-full py-5 text-white font-black rounded-2xl shadow-xl transition-all duration-300
                  ${isLoading || !formValido 
                    ...
                    : 'bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-700 hover:scale-[1.02] shadow-cyan-200 hover:shadow-cyan-300'}`}
              >
            ...
            <div className="text-center py-4 animate-in zoom-in-95 duration-500">
              ...
              <h3 className="text-xl font-black text-slate-900 mb-2 italic">Tudo pronto!</h3>
              <p className="text-slate-500 font-medium mb-8">Sua nova senha foi salva e já está ativa.</p>
              <Link 
                href="/login" 
                className="block w-full py-5 bg-gradient-to-r from-cyan-500 via-blue-600 to-indigo-700 text-white font-black rounded-2xl text-center shadow-lg hover:shadow-cyan-200 transition-all active:scale-95"
              >
                ACESSAR MINHA CONTA
              </Link>
            </div>
...
function RuleCheck({ label, active }: { label: string; active: boolean }) {
  return (
    <div className={`flex items-center gap-2 transition-all duration-300 ${active ? 'translate-x-1' : ''}`}>
      ...
      <span className={`text-[10px] font-bold ${active ? 'text-emerald-500' : 'text-slate-400'}`}>
        {label} {/* Removi a classe 'uppercase' e 'tracking-tighter' aqui */}
      </span>
    </div>
  );
}