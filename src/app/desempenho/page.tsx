...
interface DesempenhoData {
  ...
  inteligencia: {
    ...
    topicos: Array<{ idTopico: number; disciplina: string; topico: string; concluidoEm: string | null; minutos: number; sessoes: number }>;
  };
  topicos: Array<{
    idTopico: number;
    idDisciplina: number;
...
export default function DesempenhoPage() {
  ...
  const marcarTopico = async (idTopico: number, concluido: boolean) => {
    setTopicoAtualizando(idTopico);
    try {
      const res = await fetch('/api/ciclos', {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ acao: 'marcar-topico', idTopico, concluido }),
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Não foi possível atualizar o tópico.');
      carregarDados();
    } catch (error) {
      setErro(error instanceof Error ? error.message : 'Não foi possível atualizar o tópico.');
    } finally {
      setTopicoAtualizando(null);
    }
  };
  ...
  return (
    <div className="relative h-screen w-full overflow-hidden bg-[linear-gradient(135deg,#eef9ff_0%,#f8fafc_38%,#ecfdf5_100%)] text-slate-600">
      ...
      <div className="relative flex h-full overflow-hidden">
        ...
        <aside className={`app-sidebar fixed inset-y-0 left-0 z-40 flex h-screen w-64 shrink-0 flex-col border-r border-white/30 bg-slate-950/90 text-white shadow-2xl shadow-slate-950/20 backdrop-blur-xl transition-transform lg:static lg:translate-x-0 ${sidebarAberta ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <main className="min-w-0 flex-1 overflow-y-auto p-4 lg:p-6">
          <header className="app-header mb-6 flex items-center justify-between rounded-[28px] border border-white/70 bg-white/75 px-4 py-3 shadow-xl shadow-slate-200/50 backdrop-blur-xl lg:px-5">
