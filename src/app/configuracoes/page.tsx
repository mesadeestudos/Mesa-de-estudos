...
import { AlertTriangle, BookOpen, Calendar, CalendarDays, ClipboardCheck, FileCheck2, KeyRound, LayoutDashboard, Lightbulb, LineChart, LogOut, Menu, MessageCircle, NotebookTabs, RefreshCw, Save, Settings, Shield, User } from 'lucide-react';
...
function Shell({ children, router, sidebarAberta, setSidebarAberta, active, onLogout }: { children: React.ReactNode; router: ReturnType<typeof useRouter>; sidebarAberta: boolean; setSidebarAberta: (open: boolean) => void; active: string; onLogout: () => void }) {
  const navItems = [
    { icon: <LayoutDashboard size={18} />, label: 'Visão Geral', active: active === 'Visão Geral', href: '/dashboard' },
    { icon: <BookOpen size={18} />, label: 'Minha Mesa', active: active === 'Minha Mesa', href: '/minha-mesa' },
    { icon: <RefreshCw size={18} />, label: 'Ciclos de estudo', active: active === 'Ciclos', href: '/ciclos' },
    { icon: <ClipboardCheck size={18} />, label: 'Questões', active: active === 'Questões', href: '/questoes' },
    { icon: <NotebookTabs size={18} />, label: 'Caderno de erros', active: active === 'Caderno de erros', href: '/caderno-erros' },
    { icon: <FileCheck2 size={18} />, label: 'Simulados', active: active === 'Simulados', href: '/simulados' },
    { icon: <MessageCircle size={18} />, label: 'Assistente IA', active: active === 'Assistente IA', href: '/assistente' },
    { icon: <CalendarDays size={18} />, label: 'Agenda', active: active === 'Agenda', href: '/agenda' },
    { icon: <LineChart size={18} />, label: 'Desempenho', active: active === 'Desempenho', href: '/desempenho' },
    { icon: <Calendar size={18} />, label: 'Revisões', active: active === 'Revisões', href: '/revisoes' },
    { icon: <Lightbulb size={18} />, label: 'Sugestões', active: active === 'Sugestões', href: '/sugestoes' },
    { icon: <Settings size={18} />, label: 'Configurações', active: active === 'Configurações', href: '/configuracoes' },
    { icon: <User size={18} />, label: 'Perfil', active: active === 'Perfil', href: '/perfil' },
  ];

  return <div className="relative h-screen w-full overflow-hidden bg-[linear-gradient(135deg,#eef9ff_0%,#f8fafc_42%,#ecfdf5_100%)] text-slate-600"><div className="pointer-events-none absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(14,165,233,0.16),transparent_32%)]" /><div className="relative flex h-full overflow-hidden">{sidebarAberta && <div className="fixed inset-0 z-30 bg-black/40 lg:hidden" onClick={() => setSidebarAberta(false)} />}<aside className={`app-sidebar fixed inset-y-0 left-0 z-40 flex h-screen w-64 shrink-0 flex-col border-r border-white/30 bg-slate-950/90 text-white shadow-2xl shadow-slate-950/20 backdrop-blur-xl transition-transform lg:static lg:translate-x-0 ${sidebarAberta ? 'translate-x-0' : '-translate-x-full'}`}><div className="min-h-0 grow overflow-hidden px-3"><div className="px-1 pb-3 pt-4"><div className="sidebar-brand rounded-[20px] border border-white/10 bg-white/95 px-4 py-2.5 shadow-xl shadow-sky-950/20"><img src="/logo_azul.png" alt="Logo" className="mx-auto h-16 w-auto" /></div></div><nav className="space-y-1">{navItems.map(item => <MenuItem key={item.label} icon={item.icon} label={item.label} active={item.active} onClick={() => { router.push(item.href); setSidebarAberta(false); }} />)}</nav></div><div className="p-4"><button onClick={onLogout} className="group flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-bold text-slate-300 transition-all hover:bg-red-500/15 hover:text-red-200"><div className="rounded-lg bg-white/10 p-1.5 transition-colors group-hover:bg-red-500/20"><LogOut size={18} /></div>Sair</button></div></aside><main className="min-w-0 flex-1 overflow-y-auto p-4 lg:p-6"><button onClick={() => setSidebarAberta(true)} className="mb-4 rounded-xl p-2 text-slate-500 hover:bg-white/70 lg:hidden"><Menu size={20} /></button>{children}</main></div></div>;
}
