...
export default function Sidebar({ active, mobileOpen = false, onNavigate, onLogout }: SidebarProps) {
  ...
  return (
    <aside className={`app-sidebar fixed inset-y-0 left-0 z-40 flex h-screen w-64 shrink-0 flex-col border-r border-white/30 bg-slate-950/90 text-white shadow-2xl shadow-slate-950/20 backdrop-blur-xl transition-transform lg:static lg:translate-x-0 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'}`}>
